...
public class Emitter implements Control, Cloneable {
	public static enum BillboardMode {
		...
		UNIT_Z, 
		/**
		 * Particles are oriented around a common, typically fixed direction vector (see common_direction), 
		 * which acts as their local Y axis. The billboard rotates only around this axis, giving the particle some 
		 * sense of direction. Good for rainstorms, starfields etc where the particles will traveling in one 
		 * direction - this is slightly faster than oriented_self. 
		 */
		Oriented_Common, 
		/**
	...
	private String name;
	private RenderQueue.Bucket bucket = RenderQueue.Bucket.Translucent;
	...
	private int totalParticlesThisEmission, particlesPerEmission;
	...
	Vector3f commonUpVector = new Vector3f( 0,1,0);
	Vector3f commonDirection = new Vector3f( 0,0,1);
	...
	public Emitter() {
		...
		emitterNode.setShadowMode(ShadowMode.Off);
		emitterTestNode = new Node();
		emitterTestNode.setShadowMode(ShadowMode.Off);
		particleNode = new Node();
		particleNode.setShadowMode(ShadowMode.Off);
		particleTestNode = new Node();
		particleTestNode.setShadowMode(ShadowMode.Off);
	}
	...
	public void setBucket(RenderQueue.Bucket bucket) {
		this.bucket = bucket;
	}
	 ...
	 * @return 
	 */
	public float getEmissionsPerSecond() { return this.emissionsPerSecond; }
	...
	public void initialize(AssetManager assetManager) {
		if (!emitterInitialized) {
			...
			mesh.setImagesXY(spriteCols,spriteRows);
                        ...
                        if(texturePath == null) {
                            if(userDefinedMat == null) {
                                throw new IllegalArgumentException("You must set a texture.");
                            }
                            tex = userDefinedMat.getTextureParam(uniformName).getTextureValue();
                        }
                        else {
                            tex = assetManager.loadTexture(texturePath);
                            tex.setMinFilter(Texture.MinFilter.BilinearNearestMipMap);
			...
			if (particleNode.getChildren().isEmpty()) {
				...
				particleNode.setMaterial(mat);
				particleNode.setQueueBucket(bucket);
			}
